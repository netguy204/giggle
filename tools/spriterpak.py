...
def animation(res, dom):
    ...
    for key in elements(main, 'key'):
        time = attr(key, 'time', '0')

        def extract_ref(obj):
            keyid = attr(obj, 'key')
            tlid = attr(obj, 'timeline')
            parent = attr(obj, 'parent', '-1')
...
def timelines(res, dom):
    ...
    for tlxml in elements(dom, 'timeline'):
        ...
        for keyxml in elements(tlxml, 'key'):
            ...
            for objxml in keyxml.childNodes:
                ...
                values['spin'] = attr(keyxml, 'spin', '1')
                values['time'] = attr(keyxml, 'time', '0')
                values['visible'] = '1'
                values['file'], _ = os.path.splitext(fname)
                times.add(values['time'])
    ...
    for tl in result:
        diff = len(times) - len(tl)
        if diff != 0:
            # hack: replicate the last element to make up the
            # difference
            last = tl[-1]
